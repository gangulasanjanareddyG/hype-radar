...
# -----------------------------
# CONTEXT (REFINED ONLY CHANGE)
# -----------------------------
context = {
    "iPhone 17": (
        "Apple released the iPhone 17 around September 2025, which created a spike in attention as people explored new features and compared upgrades. "
        "Now that the launch period has passed, interest is gradually settling as users wait for the next cycle."
    ),
    "PS5 Pro": (
        "Interest in the PS5 Pro is driven by leaks, rumors, and expectations around performance improvements. "
        "Each new detail tends to bring more attention from gamers and tech enthusiasts."
    ),
    "Air Jordan 11": (
        "The Air Jordan 11 follows a drop-driven cycle, where attention peaks around release dates and limited restocks. "
        "This creates sharp spikes in interest followed by a gradual cooldown."
    ),
    "Owala FreeSip": (
        "The Owala FreeSip has been gaining popularity through social media, especially short-form content showing everyday use. "
        "This kind of exposure usually leads to steady and consistent growth in attention."
    ),
    "Nvidia RTX 5090": (
        "The RTX 5090 is building attention ahead of its release, driven by leaks, benchmarks, and speculation in the tech community. "
        "Pre-launch hype like this typically increases as more information becomes available."
    )
}

# -----------------------------
# HUMAN EXPLANATION (REFINED)
# -----------------------------
def human_explanation(row, product):
    ...
    if "Buy" in rec:
        return f"{product} is clearly gaining attention right now. {real} This usually happens before something takes off."
    elif "Emerging" in rec:
        return f"{product} is starting to build momentum. {real} Interest is growing steadily."
    elif "Watch" in rec:
        return f"{product} is fairly stable right now. {real} It could move either way."
    elif "Overhyped" in rec:
        return f"{product} already had strong attention earlier. {real} Now things are slowing down."
    else:
        return f"{product} is not getting much attention. {real} No strong sign of growth."
...
Confidence: <b>{confidence(row)}</b><br>
...
st.markdown(f"""
<div class='card {rec_class(rec)}'>
<h3>{rec}</h3>
<p>{rec_reason}</p>
<br>
<p><b>🧠 Why this matters:</b> {human_explanation(row, selected)}</p>
</div>
""", unsafe_allow_html=True)
...
chart_df = pd.DataFrame({
    "Signal": ["Trends","Reddit","YouTube","News"],
...
future_dates = pd.date_range(
    start=pd.to_datetime(trends['date']).iloc[-1],
    periods=len(future_vals)+1,
    freq='D'
)[1:]
